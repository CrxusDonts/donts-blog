...
public class SaTokenConfigure {
    ...
    public SaTokenConfig getSaTokenConfigPrimary() {
        ...
        config.setTokenName("access-token");             // token 名称（同时也是 cookie 名称）
        config.setTimeout(30L * 24L * 60L * 60L);       // token 有效期（单位：秒），默认30天，-1代表永不过期
        ...
        config.setIsShare(true);                    // 在多人登录同一账号时，是否共用一个 token （为 true 时所有登录共用一个 token，为 false
        // 时每次登录新建一个 token）
